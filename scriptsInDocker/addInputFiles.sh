...
echo $inputs_cmd_for_sample_files
get_var_list "link[]"
...
for file_URL in $inputs_link
	do
		f_URL=${file_URL:1:-1}
		echo $f_URL
		wget -O input${inputs_nr} $f_URL
		inuts_nr=$((++inputs_nr))
		if [[ $inputs_unpack == true ]]
			then
